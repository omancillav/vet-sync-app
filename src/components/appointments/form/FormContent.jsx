...
export function FormContent() {
  ...
  const onSubmit = async (data) => {
    try {
      // Convertir servicio_id a number según el schema
      const formData = {
        ...data,
          ...
          <Controller
            ...
            render={({ field: { onChange, value } }) => (
              <Select value={value} onValueChange={onChange} disabled={petsLoading || pets.length === 0}>
                <SelectTrigger className={`${errors.mascota_id ? 'border-red-500' : ''}`}>
                  <SelectValue
                    placeholder={
                      petsLoading
                        ? 'Cargando mascotas...'
        ...
        <div className="grid gap-2">
          ...
          <Controller
            ...
            render={({ field: { onChange, value } }) => (
              ...
              >
                <SelectTrigger className={`${errors.servicio_id ? 'border-red-500' : ''}`}>
                  <SelectValue
                    placeholder={
                      servicesLoading
                        ? 'Cargando servicios...'
